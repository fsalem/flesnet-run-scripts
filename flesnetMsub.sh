...
#echo -e "log-directory = jobs/$MOAB_JOBID\n\n" >> $FILENAME
cat flesnet.cfg.template >> $FILENAME

echo "flesnet.cfg is generated JOB_ID=$MOAB_JOBID"

if [ "1" -eq "$MULTI" ]; then
	MULTI=$MULTI COMPUTE=$COMPUTE INPUT=$INPUT SRUN=0 JOB_ID=$MOAB_JOBID HUGE_PAGES=$HUGE_PAGES aprun -N$PES_PER_NODE -n$PES_COUNT -F exclusive -d 5 ./flesnetRun.sh & # > output.out 2>&1
else
	MULTI=$MULTI COMPUTE=$COMPUTE INPUT=$INPUT SRUN=0 JOB_ID=$MOAB_JOBID HUGE_PAGES=$HUGE_PAGES aprun -N1 -n$PBS_NUM_NODES -F exclusive -d 5 -cc depth ./flesnetRun.sh # > output.out 2>&1
fi
while true
do
